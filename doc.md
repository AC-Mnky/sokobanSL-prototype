...
- 游玩态：
  ...
  - `H`：启动/重建求解器会话。
  - `L`：切换关卡编辑器模式（on/off）。
  - `Ctrl+S`：覆盖保存当前关卡（保存当前 `state + static_state` 到当前关卡索引），并在界面下方显示一行 `Level Saved`，直到玩家下一次实质性操作。
...
- 右侧面板提供与颜色配套的有色磁盘（`D`）；默认 `data={(0,0):None}`。
- 编辑模式下右键：若存在磁盘预览栈，则在顶层预览中对当前格切换 `None`（不存在则添加，已存在则移除），并同步写回对应磁盘 `data`。
