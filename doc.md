...
- `src/level_io.py`：pickle 关卡读写、内置关卡导出；读入后对 `MonoData` 递归规范化（兼容旧 pkl 缺少新字段）。`data/sequence.md`（与 `data/levels` 同级）可选：每行先去掉所有 Unicode 空白字符；若该行以 `#` 开头则为**章节标题**（去掉行首连续 `#` 后的剩余为标题，可为空），其下一行直至下一个章节标题前的关卡名为该章节；若无任何章节行，则整文件视为一个关卡列表（行为与旧版一致）。**关卡行**若以 `*` 开头（剔除空白后），表示**高难关**，选关页该按钮为红底红字；匹配用去掉行首所有 `*` 后的字符串。关卡名与 `.pkl` 主文件名一致（同样先做空白剔除再匹配）。未出现在文件中的关卡归入选关页底部「其他」，按默认文件名排序，且不为高难。选关页按章节分段显示标题。编辑器 `Ctrl+S` 按关卡名写入对应 pkl。
...
- 选关态：
  - 鼠标左键：点击关卡按钮进入游玩。
  - 鼠标滚轮：上下滚动关卡列表（内容超出窗口时）。
  - 在 `sequence.md` 中标记为高难的关卡按钮为红底红字（见 `level_io` 说明）。
  - `N`：导出内置关卡到 `data/levels/` 并刷新列表（每关一个 `.pkl` 文件）。
- 游玩态：
  - `Q` / `Esc`：返回选关。
  - `WASD` / 方向键：移动一步；长按：按下后超过 200ms 起每 100ms 再移动一步。
  - `R`：重置关卡（可被 `Z` 撤回）。
  - `Z`：撤回一步；长按：按下后超过 200ms 起每 100ms 再撤回一步（短按仍为一步）。
  - `H`：启动/重建求解器会话。
  - `L`：切换关卡编辑器模式（on/off）。
